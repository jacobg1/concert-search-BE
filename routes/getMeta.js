...
router.get('/meta/:artistName/:artistYear?', function (req, res, next) {
    ...
    const url = 'https://archive.org/advancedsearch.php?q=creator%3A' + searchParams + '&fl%5B%5D=identifier&fl%5B%5D=mediatype&fl%5B%5D=title&&fl%5B%5D=description&fl%5B%5D=year&sort%5B%5D=year+asc&sort%5B%5D=&sort%5B%5D=&rows=1000&page=&output=json'
    ...
    console.log('fetching from api')
            ...
            .then(( response ) => {
                ...
                // send result
                res.send( chunk(concertsOnly, 25) )
            })
            .catch(( error ) => {
                console.log( error )
            })
